...
const style=document.createElement('style');style.id='wardogs-v25-style';style.textContent='#weapon,#weapon option{background:#060b10!important;color:#fff!important;color-scheme:dark!important}';document.head.appendChild(style);
Object.entries(defaults).forEach(([id,v])=>{const e=$(id);if(e)e.textContent=v});
const core=document.createElement('script');core.src='./app-v19.js?v=25';core.dataset.boot='wardogs-v25-core';
core.onload=()=>{const map=document.createElement('script');map.src='./map-v22.js?v=25';map.dataset.boot='wardogs-v25-map';map.onload=()=>{const fix=document.createElement('script');fix.src='./map-v22-fix.js?v=25';fix.dataset.boot='wardogs-v25-interaction-fix';document.head.appendChild(fix)};document.head.appendChild(map)};
