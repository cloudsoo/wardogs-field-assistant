...
function install(){const host=$('map'),root=host?.querySelector('.v22-root');if(!host||!root||root.dataset.v24events)return;root.dataset.v24events='1';host.style.touchAction='none';root.style.touchAction='none';root.querySelector('.v22-canvas')?.style.setProperty('touch-action','none');
 document.addEventListener('wheel',e=>{if(!root.isConnected||!isMapEvent(e))return;const m=getMap();if(!m)return;e.preventDefault();e.stopImmediatePropagation();const v=metrics(m),r=root.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top,c=s2w(v,mx,my),old=v.q.zoom;v.q.zoom=Math.max(0,Math.min(MAX,old+(e.deltaY<0?1:-1)));const n=metrics(m),p=w2s(n,c.x,c.y);v.q.px+=mx-p.x;v.q.py+=my-p.y;clamp(n);refresh()},{passive:false,capture:true});
 ...
 const up=e=>{if(!pointer.has(e.pointerId))return;e.preventDefault();e.stopImmediatePropagation();pointer.delete(e.pointerId);const m=getMap(),d=drag;pointer.delete(e.pointerId);if(pointer.size<2)pinch=null;if(d&&d.id===e.pointerId&&m){const moved=Math.hypot(e.clientX-d.startX,e.clientY-d.startY);if(moved<6)setPointAt(metrics(m),e.clientX-root.getBoundingClientRect().left,e.clientY-root.getBoundingClientRect().top)}if(root.releasePointerCapture)try{root.releasePointerCapture(e.pointerId)}catch{}drag=null};root.addEventListener('pointerup',up,true);root.addEventListener('pointercancel',up,true);
}
function isMapEvent(e){const host=$('map'),root=host?.querySelector('.v22-root');return !!(host&&root&&root.isConnected&&root.contains(e.target)&&!e.target.closest?.('.v22-marker'))}
