...
function wireButtons(){for(const [id,fn] of [['fitMap',()=>{const q=getMap()?.q;if(q){q.zoom=0;q.px=0;q.py=0;refresh()}}],['zoomIn',()=>{const q=getMap()?.q;if(q){q.zoom=Math.min(MAX,Number(q.zoom||0)+1);refresh()}}],['zoomOut',()=>{const q=getMap()?.q;if(q){q.zoom=Math.max(0,Number(q.zoom||0)-1);refresh()}}]]){const e=$(id);if(!e||e.dataset.v24)return;e.dataset.v24='1';e.addEventListener('click',ev=>{ev.preventDefault();ev.stopImmediatePropagation();fn()},true)}}
function setPointAt(v,x,y){const s=getMap()&& (window.Wardogs19||window.Wardogs11);if(!v||!s)return;const c=s2w(v,x,y);if(c.x<v.q.view.minX||c.x>v.q.view.maxX||c.y<v.q.view.minY||c.y>v.q.view.maxY)return;if(s.mode==='target'){if(s.targetLocked)return;s.target=c}else{if(s.gunLocked)return;s.gun=c}s.mapData=v.q.data;refresh()}
function eventInsideMap(e){const host=$('map');if(!host)return false;const el=document.elementFromPoint(e.clientX,e.clientY);return !!(el&&host.contains(el))}
...
function install(){const host=$('map'),root=host?.querySelector('.v22-root');if(!host||!root||root.dataset.v24events)return;root.dataset.v24events='1';host.style.touchAction='none';root.style.touchAction='none';root.querySelector('.v22-canvas')?.style.setProperty('touch-action','none');
 if(!wheelBound){wheelBound=true;window.addEventListener('wheel',e=>{if(!eventInsideMap(e))return;const m=getMap();if(!m)return;const v=metrics(m),r=host.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;if(mx<0||my<0||mx>r.width||my>r.height)return;e.preventDefault();e.stopImmediatePropagation();const c=s2w(v,mx,my),old=v.q.zoom;v.q.zoom=Math.max(0,Math.min(MAX,old+(e.deltaY<0?1:-1)));const n=metrics(m),p=w2s(n,c.x,c.y);v.q.px+=mx-p.x;v.q.py+=my-p.y;clamp(n);refresh()},{passive:false,capture:true})}
 root.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;if(e.target.closest?.('.v22-marker'))return;e.preventDefault();e.stopImmediatePropagation();pointer.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointer.size===2){const a=[...pointer.values()];pinch={d:Math.hypot(a[0].x-a[1].x,a[0].y-a[1].y),z:getMap()?.q?.zoom||0};drag=null}else{root.setPointerCapture?.(e.pointerId);drag={id:e.pointerId,startX:e.clientX,startY:e.clientY,px:getMap()?.q?.px||0,py:getMap()?.q?.py||0};}},true);
 root.addEventListener('pointermove',e=>{if(!pointer.has(e.pointerId))return;e.preventDefault();e.stopImmediatePropagation();pointer.set(e.pointerId,{x:e.clientX,y:e.clientY});const m=getMap();if(!m)return;const q=m.q;if(pinch&&pointer.size===2){const a=[...pointer.values()],d=Math.hypot(a[0].x-a[1].x,a[0].y-a[1].y);q.zoom=Math.max(0,Math.min(MAX,pinch.z+Math.log2(Math.max(1,d)/Math.max(1,pinch.d))));clamp(metrics(m));refresh();return}if(drag?.id===e.pointerId){q.px=drag.px+e.clientX-drag.startX;q.py=drag.py+e.clientY-drag.startY;clamp(metrics(m));refresh()}},true);
 const up=e=>{if(!pointer.has(e.pointerId))return;e.preventDefault();e.stopImmediatePropagation();pointer.delete(e.pointerId);const m=getMap(),d=drag;if(pointer.size<2)pinch=null;if(d&&d.id===e.pointerId&&m){const moved=Math.hypot(e.clientX-d.startX,e.clientY-d.startY);if(moved<6)setPointAt(metrics(m),e.clientX-root.getBoundingClientRect().left,e.clientY-root.getBoundingClientRect().top)}if(root.releasePointerCapture)try{root.releasePointerCapture(e.pointerId)}catch{}drag=null};root.addEventListener('pointerup',up,true);root.addEventListener('pointercancel',up,true);
}
